...
test.describe('payments status review workflow', () => {
  test('validates, saves and applies a persisted status review', async ({ page }) => {
    ...
    await expect(page.getByText('Add a note of at least 12 characters.')).toBeVisible();
    await expect(page.getByText('Enter the reviewer or role.')).toBeVisible();

    await page.getByLabel('Status review', { exact: true }).selectOption('Blocked');
    await expect(page.getByText('Unsaved changes')).toBeVisible();

    await page.getByLabel('Next action').fill('Escalate cutover dependency');
    await page.getByLabel('Review note').fill('Reviewed with release lead and dependency owner.');
    ...
    await expect(page.getByLabel('Selected record detail')).toContainText('Blocked');
  });

  test('keeps the workflow controls reachable by keyboard', async ({ page, browserName }) => {
    await page.goto('/payments');
    await expect(page.getByRole('heading', { name: 'Migration Readiness' })).toBeVisible();
    ...
    await expectReachableByTab(page, page.getByLabel('Next action'), 'next action field', browserName);
    await expectReachableByTab(page, page.getByLabel('Review note'), 'review note field', browserName);
    await expectReachableByTab(page, page.getByLabel('Reviewed by'), 'reviewed by field', browserName);
    await expectReachableByTab(
      page,
  ...
  test('captures desktop and mobile workflow smoke screenshots', async ({ page }, testInfo) => {
    await page.setViewportSize({ height: 960, width: 1440 });
    await openPaymentsWorkflow(page);
    await page.getByLabel('Status review', { exact: true }).selectOption('AtRisk');
    await page.getByLabel('Review note').fill('Preparing a visual smoke state for workflow review.');
    await expect(page.getByText('Unsaved changes')).toBeVisible();
    ...
    const detailBox = await page.getByLabel('Selected record detail').boundingBox();
    expect(detailBox?.x).toBeGreaterThanOrEqual(0);
    expect(detailBox ? detailBox.x + detailBox.width : 0).toBeLessThanOrEqual(390);
