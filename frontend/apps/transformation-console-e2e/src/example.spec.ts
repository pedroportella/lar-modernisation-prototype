...
test('renders dashboard workstreams from the API boundary', async ({
  page,
}) => {
  await page.goto('/');
...
test.describe('feature slices', () => {
  const routes = [
    [
      ...
      'Migration Readiness',
      'Token migration',
      mockPaymentReadiness.length,
    ],
    [
      '/warehouse',
      'Operational Signals',
      'Pick exceptions',
      mockWarehouseSignals.length,
    ],
    [
      '/hr-platform',
      'Platform Tasks',
      'Role mapping',
      mockHrPlatformTasks.length,
    ],
    [
      '/insights',
      'Decision Metrics',
      'Decision latency',
      mockInsightMetrics.length,
    ],
    [
      '/automation',
      'Opportunity Queue',
  ...
  for (const [route, heading, expectedRecord, recordCount] of routes) {
    test(`renders ${route} with API records`, async ({ page }) => {
      ...
      await expect(
        page.getByRole('button', { name: expectedRecord }),
      ).toBeVisible();
      await expect(
        page.getByText('Unable to reach the backend API.'),
      ).toBeHidden();
    });
  }

  test('filters feature records and updates the detail panel', async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole('button', { name: 'Token migration' }),
    ).toBeVisible();
